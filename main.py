...
def rsi_signal(df):
    ...
    for i in range(45,len(df)-1):
        ...
        if (((abs(df['rsi'][i] - df['rsi_wma'][i]) < 3) and (abs(df['rsi'][i] - df['rsi_ema'][i])) < 3) or ((abs(tmp - df['rsi_wma'][i]) < 3.5) and (abs(tmp - df['rsi_ema'][i]) < 3))) and (abs(df['rsi_ema'][i] - df['rsi_wma'][i]) < 3.5) :
